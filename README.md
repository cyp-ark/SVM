...
### 4) Multiclass SVM
지금까지 서포트 벡터 머신을 이용해서 두 범주의 점들을 분류하는 방법에 대해 알아보았다. 서포트 벡터 머신은 본래 선형 분류기지만 커널 함수를 이용한 고차원 맵핑을 통해 비선형 분류기로써 기능 할 수 있었다. 그렇다면 두 범주가 아닌 세 범주 이상의 점들도 서포트 벡터 머신을 이용해서 분류를 할 수 있지 않을까? 이를 다음 예시를 통해 한번 확인해보자.
```python
...
서포트 벡터 머신을 다범주 분류기로 활용하는 방법은 간단하다. 예시와 같이 세 범주의 데이터가 있다는 일 대 나머지의 형태로 분류할 수 있는 선형 분류기를 3개를 만들면 되는 것이다. 실제로 싸이킷런 패키지를 통해 서포트 벡터 머신을 적용시켰을 경우 예상과 같을지 아래의 코드를 통해 확인해보자.
```python
...
<p align="left"> 예상과 같이 세 범주에 대해 각각 분류 경계면이 하나씩, 총 세개의 분류 경계면이 만들어진 것을 알 수 있다. 이를 통해 서포트 벡터 머신을 단순히 두 범주만을 분류하는데 이용할 뿐만 아니라 다 범주 분류기로써도 사용할 수 있다.


## 3. 결론
이번 튜토리얼을 통해 우리는 서포트 벡터 머신을 가장 간단한 이범주 선형 분류기부터 비선형 분류기, 커널 함수를 이용한 커널 기반 분류기, 다범주 분류기 까지 확장시켜나가는 과정을 파이썬 코드와 시각화 결과를 통해 확인해보았다. 이를 통해 일반적으로 분류문제에서 어떠한 데이터 셋을 만나더라도 서포트 벡터 머신을 적용할 수 있을 것이며, 각 하이퍼 파라미터가 가지는 의미를 이해함과 동시에 최적의 조합을 찾을 수 있을 것이다.
